defmodule EasyBillsWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, EasyBillsWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      EasyBillsWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "redirect_if_user_is_authenticated/2" do
    test "redirects if user is authenticated", %{conn: conn, user: user} do
      ...
      assert conn.halted
      assert redirected_to(conn) == ~p"/dashboard/invoices"
    end
